        ...
        while($class_id = mysqli_fetch_assoc($res_class))
        {
            ...
            <div class="accordion-item">
              <h2 class="accordion-header">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?php echo $class_id['class_id']; ?>" aria-expanded="true" aria-controls="collapseOne">
                  Завдання<?php echo " ".$class_id['class_title']." "; ?>клас
                </button>
              </h2>
              <div id="collapse<?php echo $class_id['class_id']; ?>" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                      ...
                      <div class="row">
                          <div class="col-md-3">
                              ...
                              <div class="task-list-user">
                                  ...
                                  $sql = "SELECT task_title FROM taskclass WHERE class_id = ?";
                                  $stmt = $conn->prepare($sql);
                                  $stmt->bind_param("s", $class_id['class_id']);
                                  $stmt->execute();
                                  $result = $stmt->get_result();
                                  while ($row = $result->fetch_assoc()) {
                                      $url = "admin_task_list.php?task=" . urlencode($row['task_title']);
